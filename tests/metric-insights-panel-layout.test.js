...
test('metric insights panel lives under the core metrics section and keeps a compact foldable layout', () => {
  ...
  assert.match(panelSource, /className="flex items-center gap-2"/)
  assert.match(panelSource, /className="mt-2 line-clamp-2 text-\[11px\] leading-relaxed text-slate-500"/)
  assert.doesNotMatch(panelSource, /truncate text-\[11px\] text-slate-500/)
