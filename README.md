...
- **运行时真源（source of truth）**：页面展示、历史趋势、历史阶段都只读取这一个文件
...
- 用途：
  ...
  - 历史记录
  - 饮食微调输入
  - 历史判断快照（`decisionSnapshot`）

### 周计划 / 执行模板
- 训练、饮食、护肤计划来自 `src/data/` 下的生成文件与 `src/data/dailyPlans.js`
- 这些文件属于**执行层模板真源**：浏览器运行时不会直读 Obsidian
...
- **静态体测数据文件**（`measurements.json`，运行时唯一真源）
- **本地周计划配置**（sync 后写入 generated JSON）
- **前端规则计算**
- **历史判断快照固化**（避免改规则后把旧记录整体改写）
- **重新构建部署**
...
└── utils/
    ...
    ├── historyDecisions.js         # 历史判断快照与回放优先级
    ├── metricGuidance.js           # 指标分级 / 风险 / 运动饮食说明
    ├── rulesEngine.js              # 规则引擎
